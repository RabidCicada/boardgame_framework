...
class Cell():
    ...
    def __init__(self, uid=None, name=None, attributes=None, coord=None, connections=None, **kwargs):
        ...
        #self.coord_system_class = CoordinateSystemMgr.get_system_type_class(self.cell_type) if cell_type else None

    def __str__(self):
        return f"{type(self).__name__}-{self.uid}-'{self.name}'-{len(self.children)}-COORD:{self.coord}"
    ...
    def get_coord(self,coord_sys_id):
        return self.coords[coord_sys_id]
    ...
    def get_neighbors(self, coord_system_id):
        ...
        for dir in coord.system.dirs:
            ncell = CellMgr.cells_by_coord(coord+dir)
            if ncell:
                neighs.append(ncell)
        return neighs
    ...
    def get_neighbor(self, coord_system_id, side):
        coord = self.get_coord(coord_system_id)
        return CellMgr.cells_by_coord(coord+coord.system.dirs[side])
...
class CellMgr():
    ...
    def create_auto_cells(parent_cell,cell_dict):
        ...
        if parent_cell:
            parent_cell.add_children(cells)
            for cell in cells:
    ...
    def create_cell(cls,cell_dict,parent_cell=None):
        ...
        if "cell_type" in cell_dict:
            coord=CoordinateSystemMgr.create_coord_system(cell_dict["cell_type"],(cell_dict["name"],"Local")).get_origin()
    ...
    @classmethod
    def load_seams(cls,type,system_id,raw_system,integrated_system,connections):
        ...
        if type == "auto":
            ...
            for conn in connections:
                    logger.debug(F"Seaming with {conn}")
                    ...
                    #logger.debug(F"cell1_cells:{cell1_cells}")
                    ...
                    cell2_cells = cls.by_id((conn[1][0],"Local"))
                    #logger.debug(F"cell2_cells:{cell2_cells}")
                    ...
                    #logger.debug(F"epc1:{c1} epc2:{c2}")
                    ...
                    if not seamed_coord_sys.system_tuple in cls.cells_by_coord_tuple:
                        ...
                        cell1_cells = cls.by_id((conn[0][0],"Local"))
                        for cell in cell1_cells:
                            cell.assign_default_coord(seamed_coord_sys.duplicate_coord(cell.coord))
                        logger.debug(F"Directly subsumed cells to seed {seamed_coord_sys.system_tuple} Coordinate System:{cell1_cells}")
                        ...
                        logger.debug(F"EdgeDetails for {conn[0][0]}: localedge:{conn[0][2]} glblcnt:{mapping_offset} glbledge: {seamededge_idx}")
                    ...
                    # logger.debug(F"DirXForm:{vec_xform}")
                    ...
                    if not c1.get_coord(seamed_coord_sys.system_tuple):
                        raise RuntimeError(F"Endpoint 1 in {conn} does not have a {seamed_coord_sys.system_tuple} coordinate")
                    c2.assign_default_coord(c1.get_coord(seamed_coord_sys.system_tuple) + seamed_coord_sys.dirs[seamededge_idx])
                    new_anchor = c2.coord  # Anchor coord in seamed/global system
                    #logger.debug(F"Directly Remapped epc2 coord {c2.coord} old default coord {foreign_anchor}")
    ...
    def create_subcells(cls,parent_cell,cell_dict):
        ...
        if "auto_cells" in cell_dict:
            ...
            logger.debug(F"Preparing to xform for {cell_dict['name']} (cell cnt:{len(auto_cells)})")
            ...
            logger.debug(F"Cell Summary for {cell_dict['name']}:\nNumCells:{len(auto_cells)}")
...
def auto_connect(data, cells, coord_system_id=None):
    ...
    for cell in cells:
        for side,dir in enumerate(coord_system.dirs):
            conn_cell = CellMgr.by_coord(cell.get_coord( coord_system_id)+dir )
            if conn_cell:
